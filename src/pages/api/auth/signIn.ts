...
export async function signIn(message) {
    ...
    if (message.account.providerAccountId) {
        ...
        WHERE  NOT EXISTS (
            ...
            ) AND i.email=$1 RETURNING *`, [message.account.providerAccountId]);
        ...
        await pool.query(`DELETE FROM invite WHERE email=$1`, [message.account.providerAccountId])
        // search for any stripe transactions by email, and assign them
        // next-auth creates user id on first login
        // need stripe rls
        const stripe = await pool.query(`SELECT * FROM stripe WHERE email = $1`, [message.account.providerAccountId]);
        if (stripe.rows.length > 0) {
            await pool.query(`UPDATE stripe SET user_id = $1 WHERE email = $2`, [message.user.id, message.account.providerAccountId])
        }
    }
    pool.end();
}
